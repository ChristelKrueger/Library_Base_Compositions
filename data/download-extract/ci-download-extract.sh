...
while IFS='$\n' read -r line; do
    ...
    output="$(python3 ./data/download-extract/sample_srr.py $srr_number 2 100 | cargo run --bin --release extract_comp -- --stdin --stdout --tsv --trim 50 100)" || echo "ERROR encountered on $srr_number"
